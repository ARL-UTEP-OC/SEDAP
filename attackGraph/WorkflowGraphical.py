...
class WorkflowGraphical:
    ...
    def __init__(self):
        #Set the Glade file
        self.gladefile = "WorkflowGraphical.glade"  
        ...
        #data converter
        self.routes = AttributesExtractor()
        self.attr = AttributeConverter()
        self.new()
        self.rules = ModelConverter()
        self.model = ModelEvaluator()
        self.AGInputGenerator = AGInputGenerator()
    ...
    def on_open_hacl_clicked(self, widget):
        ...
        if response == gtk.RESPONSE_OK:
            filename = dialog.get_filename()
            self.textViewHacl.get_buffer().set_text(self.AGInputGenerator.readHacl(filename))
    ...
    def on_open_evaluated_flow_clicked(self, widget):
        ...
        if response == gtk.RESPONSE_OK:
            filename = dialog.get_filename()
            self.textViewEvaluatedFlowAttributes.get_buffer().set_text(self.AGInputGenerator.readEvaluatedFlow(filename))
    ...
    def on_convert_to_P_clicked(self, widget):
        # textViewP
        try: 
            self.textViewP.get_buffer().set_text(self.AGInputGenerator.convert())
    ...
    def on_save_as_P_clicked(self, widget):  
        ...
        if path != "None":
            self.AGInputGenerator.writeOutput(path)
